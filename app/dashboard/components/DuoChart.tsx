...
export function DuoChart({
    ...
    // 1. Détection dynamique pour savoir si c'est un trimestre (ex: "T1", "T2")
    const isQuarter = periodLabel.startsWith("T");

    // 2. Définition des titres selon la période
    const evolutionTitle = isQuarter ? "Évolution Trimestre-1" : "Évolution Mois-1";
    const labelEvolShort = isQuarter ? "Évol. T-1" : "Évol. M-1";

    const volumeData = [
        { name: periodLabel, value: kpi.mois },
        { name: "Cumul", value: kpi.cumul },
    ];

    const evolutionData = [
        { name: labelEvolShort, value: kpi.evolMois ?? 0 },
    ...
    return (
        <div className="bg-white rounded-2xl border border-[#E1E8E6] p-5">
            ...
            <div className={hideEvolution ? "" : "grid grid-cols-2 gap-4"}>
                ...
                {!hideEvolution && (
                    <div>
                        {/* 3. Le titre s'adapte automatiquement ici ! */}
                        <p className="text-[10px] text-[#8AA0AA] uppercase font-bold mb-1 tracking-wide">{evolutionTitle}</p>
                        <ResponsiveContainer width="100%" height={140}>
